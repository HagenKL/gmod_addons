...
function GetFakeTesterPlayer()
	...
	for k,v in pairs(player.GetAll()) do
		if v:IsTerror() and (v:GetTraitor() or v:GetRole() == ROLE_INNOCENT or (v.IsEvil and v:IsEvil())) and !v:GetNWBool("RTTested") then
...
function SWEP:HandleFakeMessages(ply)
	...
	timer.Create("FT Timer "..id,self.Delay,1, function()
		...
		do
			local roleString,ownerRoleString=role==ROLE_INNOCENT and "innocent" or (role==ROLE_TRAITOR or (_G.IsRoleEvil and IsRoleEvil(role))) and "traitor",ownerRole==ROLE_INNOCENT and "innocent" or (ownerRole==ROLE_TRAITOR or (_G.IsRoleEvil and IsRoleEvil(ownerRole))) and "traitor" or "detective"
...
local function GetFakeRoleColor(role,ply,isOwner)
	...
	else return !(IsValid(ply)&&ply:IsTerror()) and COLOR_ORANGE or role==ROLE_INNOCENT and COLOR_RED or (role==ROLE_TRAITOR or (_G.IsRoleEvil and IsRoleEvil(role))) and COLOR_GREEN end
end
...
if CLIENT then
	...
	net.Receive("ft result", function()
		...
		if valid then
			local roleString=role==ROLE_INNOCENT and "a traitor" or (role==ROLE_TRAITOR or (_G.IsRoleEvil and IsRoleEvil(role))) and "an innocent"
