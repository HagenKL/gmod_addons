...
function SWEP:PrimaryAttack()
   if not self.Weapon:CanPrimaryAttack() then return end
	...
	if ( IsValid( ply ) and IsValid( TargetPly ) and TargetPly:IsPlayer() and TargetPly:Alive() ) then
		if( SERVER ) then
			local Pigeon = ents.Create( "ttt_pigeon" )
			if not IsValid( Pigeon ) then return end
			Pigeon:SetPos( ply:GetShootPos() + ply:GetAimVector() * 5)
			Pigeon:SetAngles( ( TargetPly:GetShootPos() - self.Owner:GetShootPos() ):Angle() )
			Pigeon.Target = TargetPly
			Pigeon:Spawn()
			Pigeon:SetOwner( ply )
			self:Remove()
			self.Owner:StripWeapon( "weapon_ttt_homingpigeon" )
