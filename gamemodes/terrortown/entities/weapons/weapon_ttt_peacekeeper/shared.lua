...
function SWEP:Think()
	if GetRoundState() == ROUND_ACTIVE then
		...
		for k,v in pairs(player.GetAll()) do
			...
			if v:IsTerror() and self.Owner:IsLineOfSightClear(v) and v != self.Owner and !v:GetNWBool("highnoonhit") and v:GetNWBool("highnoonpositionscreen") and !v:GetNWBool("DRDead") then
				if table.Count(highnoontargets) < 6 then
					if self.Owner:GetRole() == ROLE_TRAITOR then 
						if v:GetRole() != ROLE_TRAITOR then 
							table.insert(highnoontargets,v) 
						end
					elseif self.Owner:GetRole() == ROLE_DETECTIVE or self.Owner:GetRole() == ROLE_INNOCENT then
						table.insert(highnoontargets,v) 
					end
				end
...
function SWEP:Deploy()
	...
	if !IsValid(ply) or !IsValid(wep) or !wep:GetClass()=="weapon_ttt_peacekeeper" then return end
	self.Weapon:SendWeaponAnim(ACT_VM_DRAW)
	self.canusehighnoon = false
	self.highnoonactive = false
	self.highnoonshooting = false
	ResettinHighNoon(self.Owner)
	timer.Create("dontusehighnoontooearly".. self.Owner:EntIndex() , 0.75, 1, function() self.canusehighnoon = true end)
end
...
function SWEP:PrimaryAttack()
	if SERVER then
		for k,v in pairs(player.GetAll()) do
			if v != self.Owner then
				if v:GetNWBool("ItsHighNoon") or v:GetNWBool("ItsHighNoonshooting") then
					net.Start("HNFailed")
					net.Send(self.Owner)
					return 
				end
	...
	if self.canusehighnoon and GetRoundState() == ROUND_ACTIVE then
		...
		elseif !self.highnoonactive and !self.canpressattackhighnoon and !self.highnoonshooting then
			...
			for k,v in pairs(player.GetAll()) do
				v:SetNWInt("HighnoonRadius", v:Health() + 10)
				v:SetNWInt("HighnoonDamage", 1 )
			...
			timer.Create("canusehighnoonnow".. self.Owner:EntIndex(), 1,1, function()	
				...
				timer.Create("highnoondamage", 0.02, 0, function()
					for k,v in pairs(highnoontargets) do
						if v:IsTerror() and self.Owner:IsLineOfSightClear(v) and v != self.Owner and !v:GetNWBool("highnoonhit") then
							if v:GetNWInt("HighnoonRadius") > 10 then
								v:SetNWInt("HighnoonRadius", v:GetNWInt("HighnoonRadius")  - 1 ) 
							elseif v:GetNWInt("HighnoonRadius") <= 10 then
								v:SetNWInt("HighnoonRadius", 7 ) 
								v:SetNWBool("IsHighnoonfinished", true)
							end
							v:SetNWInt("HighnoonDamage", v:GetNWInt("HighnoonDamage") + 1 ) 
						end
					end
				end )
			end)
			timer.Create("highnoonisover".. self.Owner:EntIndex(), 10, 1, function()
				if self.Owner:GetNWBool("ItsHighNoon") then
...
local ply,wep=self.Owner,self.Weapon
	if !IsValid(ply) or !IsValid(wep) or !wep:GetClass()=="weapon_ttt_peacekeeper" then return end
		...
		if self.HNTarget ~= nil and self.HNTarget:IsPlayer() then
			self.Weapon:EmitSound(self.Primary.Sound, 511,100,1, CHAN_AUTO)
			wep:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
			timer.Create("secondshootanimation".. ply:EntIndex(), 0.06, 1, function() wep:SendWeaponAnim(ACT_VM_IDLE_4) end) 
						
			local dmg=DamageInfo()
			dmg:SetDamage(self.HNTarget:GetNWInt("HighNoonDamage")) 
			dmg:SetAttacker(ply)
...
function ResettinHighNoon(ply)
		...
		timer.Remove("Highnoonmakedamage".. ply:EntIndex())
...
function ResettinHighNoon2()
		for k,v in pairs(player.GetAll()) do
			...
			timer.Remove("Highnoonmakedamage".. v:EntIndex())
...
hook.Add("TTTPlayerSpeed", "HighnoonSpeed" , function(ply)
	local wep=ply:GetActiveWeapon()
		if wep and IsValid(wep) and wep.highnoonactive and wep:GetClass()=="weapon_ttt_peacekeeper" then
		return 0.2
		end
end )

hook.Add("TTTPrepareRound", "HighNoonReset", function() 
	ResettinHighNoon2()
end)
...
hook.Add( "PlayerDeath", "Highnoondead", function(ply)
	...
	ply:SetNWBool("IsHighnoonfinished", false)
	ply:SetNWBool("highnoonpositionscreen", false)
	ply:SetNWInt("HighnoonRadius", 100 )
	ply:SetNWInt("HighnoonDamage", 1 )
end )

hook.Add("EntityTakeDamage", "HighNoonLife", function(target, dmg)
	if target:GetNWBool("highnoonpositionscreen") then
		local damage = dmg:GetDamage()
		target:SetNWInt("HighnoonRadius", target:GetNWInt("HighnoonRadius")  - math.Round(damage) )
	end
end )
...
if CLIENT then
	...
	function MakeAHighNoonCircle()
		if LocalPlayer():GetNWBool("HNBegin") and LocalPlayer():Alive() then 
			if GetRoundState() == ROUND_ACTIVE and LocalPlayer():IsTerror() then
				for k,v in pairs(highnoontargets) do
						if IsValid(v) then
							surface.DrawCircle( v.highnooncirclepos.x, v.highnooncirclepos.y, v:GetNWInt("HighnoonRadius") + 5, Color( 255, 1, 1, 255))
							if v:GetNWBool("IsHighnoonfinished") then
	...
	hook.Add("RenderScreenspaceEffects", "BlackandWhite", function()
		if LocalPlayer():GetNWBool("WhiteandBlackHighNoon") and LocalPlayer():Alive() then 
			if GetRoundState() == ROUND_ACTIVE and LocalPlayer():IsTerror() then
					local tbl = {
						["$pp_colour_addr"] = 0,
						["$pp_colour_addg" ] = 0,
