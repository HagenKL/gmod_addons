...
if SERVER then
	local function DoDrinkPHD(ply)
		if IsValid(ply) and ply:IsTerror() then
			if IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "ttt_perk_juggernog" or ply:GetActiveWeapon():GetClass() == "ttt_perk_staminup" then
				timer.Simple(0.5, function() if IsValid(ply) then DoDrinkPHD(ply) end end)
			else
				ply:Give("ttt_perk_phd")
				ply:SelectWeapon("ttt_perk_phd")
				ply:GetWeapon("ttt_perk_phd"):DrinkTheBottle()
			end
		end
	end
