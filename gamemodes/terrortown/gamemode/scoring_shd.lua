   ...
   if e.id == EVENT_KILL then
      ...
      elseif aid != -1 then
         if e.vic.tr then
          ...
          elseif e.vic.jk then
            scores[aid].jackals = scores[aid].jackals + 1
         elseif not e.vic.tr and not e.vic.jk then
...
function ScoreEventLog(events, scores, traitors, detectives, jackals)
   for k, s in pairs(scores) do
      ...
      for j,v in pairs(TTTRoles) do
        if not v.IsDefault then
          scores[k]["was_" .. v.String] = table.HasValue(jackals,k)
        end
      end
   end
...
function ScoreTeamBonus(scores, wintype)
   ...
   for k, sc in pairs(scores) do
      ...
      elseif sc.was_jackal then
         state.jackals = state.jackals + 1
      else
...
function KillsToPoints(score, was_traitor, was_jackal)
   local points = ((score.suicides * -1) + score.bonus + (score.traitors * (was_traitor and -16 or was_jackal and 3 or 5)) + (score.innos * (was_traitor and 1 or was_jackal and 4 or -8)) + (score.deaths == 0 and 1 or 0))

   for k,v in pairs(TTTRoles) do
     if v.newteam then
       points = points + (score[v.String .. "s"] * (was_jackal and -2 or 3))
     end
   end
